...
export default function SettingsScreen() {
  ...
  const handleDeleteAvatar = () => {
    removeAvatarSheet.open();
  };

  const confirmRemoveAvatar = () => {
    deleteAvatarMutation.mutate(undefined, {
      onSuccess: async () => {
        removeAvatarSheet.close();
        await refreshUser();
  ...
  const handleDeleteMess = () => {
    deleteMessSheet.open();
  };

  const confirmDeleteMess = () => {
    deleteMessMutation.mutate(undefined, {
      onSuccess: async () => {
        deleteMessSheet.close();
        await clearMess();
        router.replace("/(onboarding)/create-mess");
  ...
  return (
    <>
        ...
        {messQuery.data ? (
          <FadeIn delay={150}>
            <Card variant="glass" className="mb-4">
              ...
              <Badge
                label={`${messQuery.data.activeMemberCount ?? 0} - ${messQuery.data.isActive ? "Active" : "Inactive"}`}
                variant="muted"
              />
            </Card>
          </FadeIn>
      ...
      <ConfirmSheet
        sheetRef={removeAvatarSheet.ref}
        title="Remove photo"
        description="Delete your profile photo? This cannot be undone."
        confirmLabel="Remove"
        variant="danger"
      ...
      <ConfirmSheet
        ...
        description={
          messQuery.data
            ? `Permanently delete "${messQuery.data.name}" and all its data? This cannot be undone.`
            : "Permanently delete this mess and all its data? This cannot be undone."
        }
        confirmLabel="Delete"
        variant="danger"
        loading={deleteMessMutation.isPending}
        onConfirm={confirmDeleteMess}
      />
    </>
  );
}
